...
file.create("C:/Users/Lisa/Desktop/KAOS/combined_transect_density.csv")
...
for (i in 1:length(uniqueTransect)) {
  ...
  #convert to density using target strength (units = g/m2 per interval)
  #formula is multiplied by 250m because this is the depth of slice through the water column
  p <- 250*10 ^((mvbs - -42.22)/10)*1000
  #remove p values > 1000. No integration intervals in the KAOS survey official report
  #had a denisty of over 600gm2. Remove this section when noise regions are available.
  p[p > 1000] <- NA
  
  #find the transect density as the mean of integration interval density
  p_transect <- mean(na.omit(p))
  
  #write the density per vertical slice to a single .csv file
  write.table(cbind(i, p_transect), file = "C:/Users/Lisa/Desktop/KAOS/combined_transect_density.csv", row.names = F, col.names = F, sep = ",", append = TRUE)
  
  message(paste(Sys.time(), "Finished calculating krill density for transect ", i))
}
@

Now that the mean densities for all 6 example transects are in the same .csv file, the density and biomass for the full survey area can be calculated. The following coordinates were used as bounding locations for the survey and the \texttt{geosphere} package in \texttt{R} was used to calculate the survey area. The Jolly and Hampton (1990) method for estimating survey density is used which involves weighting the transect densities by transect length.

<<calculateBiomass>>=
#Calculate average survey density
biomass.data <- read.csv("C:/Users/Lisa/Desktop/KAOS/combined_transect_density.csv", header = F)

#calculate survey area in m^2
library(geosphere)
coords <- read.csv(paste(dd, "/survey_coordinates.csv", sep =""), header = T)
survey.area <- areaPolygon(coords)

#read in transect lengths in km
transect.length <- read.csv("C:/Users/Lisa/Desktop/KAOS/transect_length.csv", header = F)

length_weight <- transect.length/sum(transect.length)

#multiply transect density by weighted transect length
p_weighted <- sum(biomass.data[, 2] * length_weight)

#multiply by survey area to get survey biomass (units = tonnes)
survey.biomass <- p_weighted * survey.area /1e6
@

The total estimated biomass for the \Sexpr{round(survey.area, 2)} square km sample area of the survey is \Sexpr{round(survey.biomass, 2)} tonnes. Note that this is only a simple example of how the package \texttt{acoustic} can be used to automatically estimate krill density. It is not intended as a reference for biomass estimation.
