...
<<schoolsDetection>>=
  for(i in 1:length(fnEVVec)){
    ...
    #export Sv data for 38kHz and 120kHz all aggregations
...
#read in data files - the results of integration by regions for each aggregation
    ##change to be for each transect!
...
swarms <- subset(ag, ag$dBdiff > 1.02 & ag$dBdiff < 14.75)
...
message("Finished classifying aggregations for transect", i, sep = " ")
}
@

This vignette has demonstrated an automated method for performing schools detection in texxt{Echoview} and then using texxt{R} to run classification on the detected aggregations.
\end{document}