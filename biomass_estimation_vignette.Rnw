...
for (i in 1:length(uniqueTransect)) {
  ...
  write.table(p, file = "C:/Users/Lisa/Desktop/KAOS/combined_density_intervals.csv", row.names = F, col.names = F, sep = ",", append = TRUE)
  
  message(paste(Sys.time(), "Finished calculating krill density for transect ", i))
}
@

Now that the densities for all 6 example transects are in the same .csv file, the biomass for the survey area can be calculated. The following coordinates were used as bounding locations for the survey and the \texttt{geosphere} package in \texttt{R} was used to calculate the survey area. Since this example is primarily to demonstrate the automation of Echoview, we will simply calculate biomass using mean survey density * survey area.

<<calculateBiomass>>=
#Calculate average survey density
biomass.data <- read.csv("C:/Users/Lisa/Desktop/KAOS/combined_density_intervals.csv", header = F)
...
survey.biomass <- mean(na.omit(biomass.data$V1)) * survey.area /1000
@
